...
def main():
    ...
    if switch == 'Train':
        ...
        for i in range(len(normalized_train_data) - time_step):
            x.append(normalized_train_data[i:i + time_step, :total_column_nums])
            y.append(normalized_train_data[i + time_step - 1, total_column_nums])
        data_train_X = np.array(x)
        data_train_Y = np.array(y)

        x1,y1= [], []
        for i in range(len(normalized_test_data) - time_step):
            x1.append(normalized_test_data[i:i + time_step, :total_column_nums])
            y1.append(normalized_test_data[i + time_step - 1, total_column_nums])
        data_test_X = np.array(x1)
        data_test_Y = np.array(y1)


        model = Sequential()
        model.add(LSTM(units=100, return_sequences=False,input_shape=(data_train_X.shape[1], data_train_X.shape[2])))
        ...
        for i in range(test_y.shape[0]):
            aa[i+test_data.shape[0]-test_y.shape[0],total_column_nums] = test_y[i,0]
        bb = scaler.inverse_transform(aa)
        for i in range(test_data.shape[0]):
            print("ori:",test_data[i,total_column_nums],"predict:",bb[i,total_column_nums])
        plt.plot(test_data[:,total_column_nums],color='red',label='Original')
    ...
    if switch == 'test':
        ...
        for i in range(pred_y.shape[0]):
            aa[i+pred_data.shape[0]-pred_y.shape[0],total_column_nums] = pred_y[i,0]
        bb = scaler.inverse_transform(aa)
        for i in range(pred_data.shape[0]):
            print("ori:",pred_data[i,total_column_nums],"trans:",bb[i,total_column_nums])
        ...
        rmse = math.sqrt(tf.losses.mean_squared_error(ori_value, predict_value))
